...
// distance is only known for static contacts; dynamic contacts fall back to 0 (always within range)
...
const staticDistances = Object.values(contactDistancesKm);
const MIN_DISTANCE    = Math.min(...staticDistances);
const MAX_DISTANCE    = Math.max(...staticDistances);
  ...
  return (
    <div className={`contacts-dropdown ${open ? "is-open" : ""}`}>
      ...
      >
        ...
        <span className="contacts-filter-btn__arrow" aria-hidden="true">v</span>
      </button>
      ...
      <div className="contacts-dropdown-menu" role="menu" aria-label={label}>
        {Object.entries(filterLabels[filterName]).map(([value, optionLabel]) => (
          <button
            className={`contacts-dropdown-option ${activeValue === value ? "is-active" : ""}`}
            key={value}
...
export default function ContactsDirectoryPage() {
  ...
  // ── seed once from static data, then load everything from localStorage ─────
  useEffect(() => {
    initContacts();
    const contacts = loadContacts();
    console.log("Public contacts:", contacts);
    setAllContacts(contacts);
  }, []);

  // ── compute price range from ALL contacts ────────────────────────────────
  const allPrices = useMemo(() => allContacts.map(getContactPrice), [allContacts]);
  const minPrice  = useMemo(() => (allPrices.length ? Math.min(...allPrices) : 0), [allPrices]);
  const maxPrice  = useMemo(() => (allPrices.length ? Math.max(...allPrices) : 999), [allPrices]);
  ...
  // initialise budgetMax to the actual price ceiling once contacts load
  useEffect(() => {
    if (maxPrice > 0) {
      setFilters((f) => ({
        ...f,
        budgetMax: f.budgetMax === 999999 ? maxPrice : Math.max(f.budgetMax, maxPrice),
      }));
    }
  }, [maxPrice]);
  ...
  const filteredContacts = useMemo(() => {
    ...
    return allContacts.filter((contact) => {
      ...
      // dynamic contacts (no entry in contactDistancesKm) default to 0 — always within range
      const distance = contactDistancesKm[contact.id] ?? 0;
      const review   = Number(contact.review || 0);

      return (
        searchable &&
        (filters.specialty === "all" || contact.specialty === filters.specialty) &&
        price <= filters.budgetMax &&
        distance <= filters.distanceMax &&
        (filters.review === "all" || review >= Number(filters.review))
      );
    });
  }, [allContacts, filters, query]);
          ...
          <div className="contacts-shell">
                ...
                <div className="contacts-filters" aria-label="Contact filters">
                  ...
                  <div className="contacts-price-filter">
                    <div className="contacts-price-filter__head">
                      <span>Budget</span>
                      <strong>Up to EGP {maxPrice > 0 ? Math.min(filters.budgetMax, maxPrice) : "..."}</strong>
                    </div>
                    <input
                      ...
                      step="10"
                      type="range"
                      value={Math.min(filters.budgetMax, maxPrice)}
                    />
              ...
              <div className="contacts-toolbar__actions">
                <div className="contacts-view-toggle" aria-label="View toggle">
                  <button
                    ...
                    className={`contacts-view-btn ${view === "grid" ? "is-active" : ""}`}
                  ...
                  <button
                    ...
                    className={`contacts-view-btn ${view === "list" ? "is-active" : ""}`}
            ...
            >
              {filteredContacts.length === 0 ? (
                ...
                filteredContacts.map((contact) => {
                  ...
                  const imgSrc     = getImgSrc(contact.image);
                  ...
                  return (
                    <article className="contact-card" key={contact.id}>
                      ...
                      <div className="contact-card__content">
                        ...
                        <span className="contact-card__status">{contact.status}</span>
                        ...
                        <div className="contact-card__info">
                          ...
                          <div className="contact-card__row">
                            <span className="contact-card__icon">&#9993;</span>
                            <span className="contact-card__email" title={contact.email}>
                              {contact.email}
                            </span>
                          </div>
                        </div>
                      ...
                      <div className="contact-card__footer">
                        <div className="contact-card__actions">
                          <button
                            ...
                            style={{
                              ...
                              cursor: "pointer",
                              padding: "4px 6px",
                              color: isFavorite ? "#e11d48" : "#d1d5db",
                              fontSize: "20px",
                              lineHeight: 1,
                        ...
                        <Link
                          ...
                          href={`/contact-details?id=${encodeURIComponent(contact.id)}`}
                        >
                          &#8250;
                        </Link>
                      </div>
            ...
            <div className="contacts-pagination-wrap">
              <p className="contacts-summary">
                Showing {filteredContacts.length ? 1 : 0}–{filteredContacts.length} from {allContacts.length} contacts
              </p>
            </div>
          </div>
        </section>
